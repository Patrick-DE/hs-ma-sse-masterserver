...
exports.Token = function (req, res, next) {
	...
	jwt.verify(token, process.env.SECRET, function (err, decoded) {
		if (err) return res.status(500).send({ auth: false, token: null, message: 'The token expired.' });

		// check if token has client ip
		if (decoded.ip === undefined || req.ip !== decoded.ip) return res.status(403).send({ auth: false, token: null, message: 'Failed to authenticate token.' });
